...
| engine | VARCHAR2(20) | NOT NULL, DEFAULT 'rule' | 판정 경로: `llm-api`(사내 LLM 응답) / `rule`(규칙 기반만 — LLM 미설정·실패) / `unavailable`(AI 서버 자체 미응답) |
| created_at | TIMESTAMP | NOT NULL | |
...
| title | VARCHAR2(200) | NOT NULL | 화면 4단계에서 입력한 변경 사유(고객 합의 내용에서 기본값으로 채워짐, 편집 가능). 비어 오면 서버가 "최초 확정"을 기본값으로 채운다 |
