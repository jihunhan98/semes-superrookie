# ai-model — 요구사항 검토 AI 서버

요구사항 문장에서 **불명확한 표현**과 **기존 요구사항과의 상충**을 찾아내는 FastAPI 서버.
Spring Boot 백엔드가 요구사항 등록/수정 시 이 서버를 호출한다.

> 사용자 입력부터 검토 결과가 화면에 뜨기까지 전체 과정을 그림으로 정리한 것:
> [`docs/img/ai-flow-detail.svg`](../docs/img/ai-flow-detail.svg)

## 검출 방식 — 규칙 기반 + 사내 LLM

**규칙 기반 검출이 항상 먼저 실행된다.** 미리 정해둔 표현이 문장에 있으면 그 자리에서
바로 잡아낸다(치환 제안까지 나온다). 그 위에 **사내 LLM API**(GPT-OSS-120B, OpenAI 호환)를
한 번 더 호출해서, 규칙이 못 잡는 것까지 보충한다.

사내 LLM이 응답하지 못해도(주소 미설정·연결 실패·타임아웃) 요청 자체는 실패하지
않는다 — 규칙 기반 결과만 담아 `engine: "rule"`로 응답한다. 그래서 요구사항
등록은 사내 LLM 상태와 무관하게 항상 끝까지 진행된다. 폐쇄망이라 외부 AI API
(Claude/GPT 등)는 애초에 쓸 수 없고, 이 사내 서비스 하나만 쓴다.

> 사내 LLM API는 OpenAI 파이썬 라이브러리와 같은 규격이라, 아래 호출과 동등하다.
> 다만 폐쇄망에 `openai` 패키지를 반입하지 않으려고 표준 라이브러리로 직접 보낸다.
>
> ```python
> client = OpenAI(api_key=LLM_API_KEY, base_url=LLM_API_BASE)
> client.chat.completions.create(model=LLM_API_MODEL, messages=[...])
> ```

## 지금 실제로 나가는 프롬프트

아래 "프롬프트 커스터마이징" 절의 템플릿은 **앞으로 채워 넣을 개선안**이고, 지금
`main.py`가 실제로 사내 LLM에 보내는 것은 이것보다 짧다 — 유형 이름만 나열하고
예시(few-shot)가 없는 버전이다. `LLM_API_BASE`가 비어 있으면 이 절 자체가
아예 호출되지 않고 규칙 기반 결과만 나간다(위 "검출 방식" 참고).

**1. system 메시지** — `main.py`의 `_SYSTEM_PROMPT` 상수, 지금 그대로:

```
당신은 반도체 장비 소프트웨어(VCS/AMR) 요구사항을 검토하는 전문가다.
주어진 요구사항 문장에서 개발자와 고객이 서로 다르게 해석할 수 있는 부분을 찾아라.

검출 유형은 다음 중 하나만 사용한다:
정량 기준 부재, 모호한 정도부사, 주어·주체 불명확, 조건 발생 시점 불명확,
예외·경계 조건 누락, 접속사 범위 모호, 시간·일정 모호

반드시 아래 JSON 형식으로만 답한다. 문제가 없으면 findings 를 빈 배열로 둔다.
{"findings":[{"findingType":"유형","targetSpan":"문제가 된 구절","reason":"왜 문제인지","suggestion":"어떻게 바꾸면 좋은지"}]}

targetSpan 은 반드시 원문에 그대로 등장하는 구절이어야 한다.
```

**2. user 메시지** — 검토할 문장 + (있으면) 수정 사유. 최초 확정은 본문 전체,
확정본 수정은 바뀐 부분만 이 자리에 들어간다(위 "검출 방식" 참고):

```
요구사항: {검토 대상 문장}
수정 사유: {reason}          ← 확정본 수정일 때만 붙는다
```

**3. 실제 HTTP 요청** — `POST {LLM_API_BASE}/chat/completions`, OpenAI Chat
Completions 규격 그대로:

```jsonc
{
  "model": "{LLM_API_MODEL}",
  "messages": [
    { "role": "system", "content": "<위 1번 그대로>" },
    { "role": "user", "content": "<위 2번 그대로>" }
  ],
  "temperature": 0
}
```

`temperature: 0`으로 고정한 이유는 같은 문장을 넣으면 같은 결과가 나오게 하려는
것이다 — 규칙 기반 결과와 마찬가지로 "다시 분석"을 눌러도 본문이 안 바뀌었으면
결과도 안 바뀌는 게 정상 동작이다(사내 LLM이 매번 표현을 바꿔주길 기대하고
있었다면, 온도를 올리는 것도 방법이지만 검증 가능성이 떨어진다).

**4. 응답을 받은 뒤** — `_to_findings()`가 이 순서로 걸러낸다:

1. 응답 텍스트에서 JSON 객체 하나를 뽑는다(앞뒤에 설명 문장이 섞여 있어도 됨).
2. `findingType`이 8종 화이트리스트에 없으면 버린다.
3. `targetSpan`이 원문에 그대로 없으면 버린다 — **환각 방지**. LLM이 원문에
   없는 구절을 지어내는 경우가 실제로 있다.
4. 남은 것을 규칙 기반 결과 뒤에 덧붙인다 — **규칙 결과가 항상 우선**이고,
   같은 `(유형, 구절)` 조합이 이미 있으면 LLM 쪽은 버려서 중복 표시를 막는다.

## 실행

```bash
cd ai-model
python3 -m venv venv
source venv/bin/activate
pip install -r requirements.txt
uvicorn main:app --host 0.0.0.0 --port 8001
```

사내 LLM API를 쓰려면 주소를 `.env`에 넣는다. **사내 서버 주소는 사내망 정보라
저장소에 두지 않는다** — `.env`는 `.gitignore`에 있어 커밋되지 않는다.

```bash
cp .env.example .env
# .env 를 열어 LLM_API_BASE / LLM_API_MODEL 을 채운다 (실제 값은 팀 내부에서 받는다)
```

주소를 안 채워도 서버는 정상 동작한다 — 규칙 기반 결과만으로 응답하므로
사내망 밖(집·외부 PC)에서도 그대로 개발할 수 있다.

## 환경 변수

| 변수 | 기본값 | 설명 |
|---|---|---|
| `LLM_API_BASE` | (없음) | **사내 LLM API** 주소 (`/v1`까지 포함). 비어 있으면 LLM 호출을 건너뛴다 |
| `LLM_API_MODEL` | `gpt-4` | 사내 서버가 서빙하는 모델명 — 서버에 맞게 조정 |
| `LLM_API_KEY` | `EMPTY` | 사내 서비스는 인증이 없어 `EMPTY` |
| `LLM_API_TIMEOUT` | `30` | 사내 LLM 응답 대기 시간(초) |
| `ANALYZE_DELAY` | `0` | **인위적 지연(초)** — 등록 화면 로딩 UI 테스트용 |

`ANALYZE_DELAY=3` 으로 띄우면 등록 시 3초 로딩이 걸려서 프론트 로딩 화면을 확인할 수 있다.

값은 `ai-model/.env`(커밋 안 됨)에 넣거나 실행할 때 환경 변수로 준다. 셸에 이미 설정된
값이 `.env`보다 우선한다. 소스에 사내 주소를 적지 않는다.

## API

### `POST /analyze`

```jsonc
// 요청 — 최초 확정(본문 전체 검토)
{
  "content": "AMR 매칭 시 가용한 AMR 중 가장 가까운 AMR을 선택한다.",
  "existing": [{ "reqKey": "req-ta-01", "content": "태스크는 요청 순으로 할당한다." }]
}

// 요청 — 확정본 수정(변경분만 검토)
{
  "content": "…최대 1건만 허용한다. 우선순위는 SoC 높은 순으로 정한다.",
  "baseContent": "…최대 1건만 허용한다.",
  "reason": "req-ta-01과 우선순위 기준이 달라 통일 요청",
  "existing": []
}
```

```jsonc
// 응답
{
  "findings": [
    {
      "findingType": "정량 기준 부재",
      "targetSpan": "가용한 AMR",
      "reason": "무엇을 '가용'으로 볼지 판정 기준이 없어 해석이 갈릴 수 있음",
      "suggestion": "가용한 AMR = IDLE 상태이며 SoC(배터리 잔량)가 최소값 이상인 AMR",
      "conflictReqKey": null
    }
  ],
  "draftContent": "AMR 매칭 시 IDLE 상태이며 SoC 최소값 이상인 AMR 중 …",
  "engine": "llm-api",   // llm-api(사내 LLM 응답) | rule(규칙 기반만 — LLM 미설정·실패)
  "scope": "full",
  "elapsedMs": 3
}
```

**규칙 결과가 먼저**이고 LLM이 새로 찾은 것만 뒤에 덧붙는다.
LLM이 원문에 없는 구절(`targetSpan`)이나 정의되지 않은 유형을 만들어내면 버린다(환각 방지).

`draftContent`가 화면의 **"확정될 본문"에 그대로 채워지는 값**이다.
AI 검토 결과 카드는 읽기 전용이고, 사용자는 이 draft 텍스트를 직접 고쳐서 확정한다.

### `GET /health`

사내 LLM API 주소가 설정되어 있는지를 반환한다. 폐쇄망에 배포한 뒤
"사내 LLM 주소가 제대로 들어갔는지" 빠르게 확인하는 용도다.

```jsonc
{ "status": "ok", "llmApiConfigured": true, "llmApiModel": "gpt-4" }
```

### `GET /types`
검출 유형 목록. 프론트 안내 문구의 유형 칩과 동일한 순서다.

## 검출 유형

`정량 기준 부재` · `모호한 정도부사` · `주어·주체 불명확` · `조건 발생 시점 불명확`
· `예외·경계 조건 누락` · `접속사 범위 모호` · `시간·일정 모호` · `기존 요구사항과 상충`

앞의 7개는 문장 자체의 문제라 `draftContent`로 치환 제안이 가능하다.
마지막 `기존 요구사항과 상충`은 문장만 고쳐서 해결되지 않으므로 치환하지 않고
협의 대상으로만 표시한다(이 유형은 LLM이 아니라 규칙 엔진이 `existing[]` 비교로
검출하므로, 아래 LLM 프롬프트에는 포함하지 않는다).

## 프롬프트 커스터마이징 — 실제 요구사항 예시로 정확도 높이기

지금 `main.py`의 `_SYSTEM_PROMPT`는 유형 이름만 나열하고 실제 예시가 없다. LLM에게
**우리 도메인의 실제 요구사항 문장을 몇 개 보여주면(few-shot)**, 판정 정확도와 제안
품질이 크게 올라간다. 아래 템플릿의 `<< >>` 부분을 실제 요구사항 문장으로 채운 뒤,
**사내 LLM API 플레이그라운드(채팅 UI)에 그대로 붙여넣어 직접 테스트**한다.

원문을 이 대화(Claude)로 보낼 필요는 없다 — 이 템플릿은 사내망 안에서 직접 채우고
직접 테스트하도록 만든 것이다. 결과가 마음에 들면 `<< >>`를 채운 최종본을
`ai-model/main.py`의 `_SYSTEM_PROMPT` 상수에 그대로 붙여넣어 교체한다.

### 템플릿

```
당신은 반도체 장비 소프트웨어(VCS/AMR) 요구사항을 검토하는 전문가다.
주어진 요구사항 문장에서 개발자와 고객이 서로 다르게 해석할 수 있는 부분을 찾아라.

검출 유형은 다음 중 하나만 사용한다:
정량 기준 부재, 모호한 정도부사, 주어·주체 불명확, 조건 발생 시점 불명확,
예외·경계 조건 누락, 접속사 범위 모호, 시간·일정 모호

각 유형의 실제 예시는 다음과 같다. 문장에서 비슷한 패턴을 찾아라.

[정량 기준 부재]
문장: << 실제 요구사항 문장 1~2개 >>
문제: << 왜 문제인지 — 판정 기준이 뭐가 없는지 >>
제안: << 어떻게 고치면 좋은지. 반드시 "ex: 대체 문구" 형태로 실제 대체 가능한 문장까지 포함 >>

[모호한 정도부사]
문장: << >>
문제: << >>
제안: << >>

[주어·주체 불명확]
문장: << >>
문제: << >>
제안: << >>

[조건 발생 시점 불명확]
문장: << >>
문제: << >>
제안: << >>

[예외·경계 조건 누락]
문장: << >>
문제: << >>
제안: << >>

[접속사 범위 모호]
문장: << >>
문제: << >>
제안: << >>

[시간·일정 모호]
문장: << >>
문제: << >>
제안: << >>

반드시 아래 JSON 형식으로만 답한다. 문제가 없으면 findings 를 빈 배열로 둔다.
{"findings":[{"findingType":"유형","targetSpan":"문제가 된 구절","reason":"왜 문제인지","suggestion":"어떻게 고치면 좋은지 — 반드시 'ex: 대체 문구' 형태로 구체적인 예시를 포함해서 답하라"}]}

targetSpan 은 반드시 원문에 그대로 등장하는 구절이어야 한다.
```

### 어떤 유형으로 분류할지 애매할 때 — 판단 기준

| 문장에 이런 게 있으면 | 검출 유형 |
|---|---|
| 숫자·기준값 없이 "적절히/충분히/가용한" 같은 판단 여지가 있는 표현 | 정량 기준 부재 / 모호한 정도부사 |
| "누가" 하는지 안 적혀 있음(부서·역할명 없이 "담당자가", "관련 부서") | 주어·주체 불명확 |
| "~시/~때/~경우" 조건은 있는데 그 조건이 언제 성립하는지 판정 기준이 없음 | 조건 발생 시점 불명확 |
| 조건은 있는데 "그 외의 경우" 동작이 안 적혀 있음 | 예외·경계 조건 누락 |
| "및/등/혹은"으로 여러 개를 묶었는데 범위가 어디까지인지 안 정해짐 | 접속사 범위 모호 |
| "신속히/즉시/주기적으로"처럼 시간·주기가 숫자로 안 나와 있음 | 시간·일정 모호 |
| 문장 하나로는 안 잡히고, 다른 요구사항과 비교했을 때만 보이는 문제(같은 기준을 서로 다르게 정함) | 기존 요구사항과 상충 (규칙 엔진이 자동 검출, 위 프롬프트 대상 아님) |

한 구절이 여러 유형에 걸쳐 보이면 가장 핵심적인 것 하나만 고른다 — 검출 유형을
여러 개 붙이면 화면이 지저분해진다.

### 예시 채우는 팁

- 유형마다 예시 2~3개면 충분하다 — 너무 많으면 프롬프트가 길어져서 사내 LLM 응답이 느려진다.
- 실제로 등록될 요구사항과 같은 어투·용어를 그대로 써야 한다. 예시에 "적절히"를 넣었는데
  실제 문서는 "적당히"를 쓴다면 LLM이 패턴을 못 잡을 수 있다.
- `suggestion`에 넣는 `ex: ...` 예시는 실제로 "확정될 본문" 화면에 참고가 되는 문구이므로,
  실무에서 그대로 쓸 수 있는 표현으로 적는다.
- 유형이 우리 도메인과 안 맞는다고 느껴지면(예: 8종 중 실제로 거의 안 걸리는 유형이 있거나,
  반대로 8종에 안 들어가는 새로운 패턴이 자주 보이면) 유형 자체를 조정하는 것도 방법이다 —
  그 경우 `rules.py`의 `ALL_TYPES`·`PHRASE_RULES`와 위 프롬프트 둘 다 같이 고쳐야 한다.

## 파일

```
ai-model/
  main.py           # FastAPI 앱 — /analyze, /health, /types
  rules.py          # 규칙 기반 검출기 (항상 먼저 실행)
  requirements.txt
  .env.example      # 설정 템플릿 — .env 로 복사해서 채운다
  .env              # 사내 LLM 주소 등 — .gitignore 되어 커밋되지 않는다
```
