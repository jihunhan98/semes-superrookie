...
- **네트워크 제약 — 반입 O / 반출 X**: 인터넷에서 받아오는 것(모델 다운로드·`git pull` 등 인바운드)은 가능하지만, **데이터를 밖으로 내보내는 것(아웃바운드)은 차단**된다. 따라서 요청에 요구사항 데이터가 실려 나가는 **외부 AI API(Claude·GPT 등)는 사용 불가** — 대신 오픈소스 모델을 사내에서 로컬 서빙한다.
- AI 모델은 사내 워크스테이션(**Quadro P4000 8GB · Xeon Gold 5122 8코어 · RAM 32GB**)에서 Hugging Face 오픈소스 모델을 로컬 서빙. VRAM 8GB 제약상 **최신 7~8B instruct 모델을 양자화(GGUF, Ollama/llama.cpp)** 로 돌리는 것이 상한선이다. **1차 후보는 Qwen3-8B(`Q5_K_M` ≈ 5.85GB)** — 8GB에 통째로 올라간다(스파이크 검증본은 `qwen2.5:7b-instruct`). 13B 이상은 CPU 오프로딩이 필요해 느려지고, 32B+는 사실상 불가.
...
## 4. 핵심 기능 3 — DB 변경 관리 · 알림 (스키마 동기화)

DB 테이블이 많고, 테스트 과정에서 담당자마다 각자 스키마를 쓰다 보니 **테이블이 바뀌어도 다른 사람이 모르고, 누락된 컬럼·값이 쌓이는 문제**가 있다.

### 4.0 스키마 구조 — 개인 vs 최종

이 프로젝트에는 두 종류의 스키마가 있다.

| 스키마 | 용도 | 비유(git) |
|---|---|---|
| **개인 스키마** | 각 개발자가 개발·테스트에 쓰는 로컬 DB | local |
| **최종 스키마** | 고객사 배포 시 들고 가는 기준 DB — **모두 여기에 맞춰야 함** | remote(master) |

핵심 목표: **최종 스키마가 바뀌면 각자 개인 스키마를 거기에 맞춰 동기화**하는 것. 이 흐름은 사실상 **git pull과 동일한 모델**이다 (최종=remote, 개인=local, "변경 적용"=pull).

![DB 스키마 동기화 — 최종 → 개인 pull](docs/img/design-fig4.png)

### 4.1 변경점 감지 (최종 vs 개인)

- Oracle **데이터 딕셔너리**(`ALL_TABLES` / `ALL_TAB_COLUMNS` / `ALL_CONSTRAINTS` 등)로 두 스키마의 구조를 읽어 **차이(diff)**를 계산한다.
- 또는 최종 스키마 변경을 **버전이 매겨진 변경 이력(changelog)**으로 관리해, "무엇이·언제·순서대로" 바뀌었는지 추적한다. (pull을 안전하게 하려면 이 방식이 유리)
- 주기적 폴링 또는 화면 조회 시 "최종은 v38인데 내 스키마는 v37" 처럼 밀린 변경을 감지한다.
...
- 변경 유형: 테이블 미사용(deprecated) / 테이블명 변경 / 컬럼 추가·변경 / 속성 변경(NOT NULL·기본값·타입) / 데이터(권한 등) 추가.
...
감지된 변경을 **버튼 한 번으로 내 개인 스키마에 반영**한다 (git pull과 동일).

- 각 변경점을 DDL/DML로 변환해 클릭 시 실행: 컬럼 추가 → `ALTER TABLE ... ADD`, 권한 행 추가 → `INSERT`, 테이블명 변경 → `RENAME` 등.
- **추가 계열(컬럼·테이블·행 추가)은 안전** → 클릭 즉시 적용.
- **삭제·타입 변경은 위험**(데이터 손실·적용 실패 가능) → **"확인 후 적용"** 단계를 둔다.
- 순서/의존성(FK), 내 스키마를 이미 다르게 바꾼 경우의 충돌은 "최종 우선 + 경고" 정책으로 처리.

### 4.4 버전 관리 · 이력

- 변경이 누적되면 스키마에 버전이 매겨져(예: `v38`) 모두가 같은 최신 버전을 본다.
- 각 변경은 "언제·누가·무엇을 바꿨는지" 이력으로 남아 추적 가능하다.
- **구현 방향**: 처음부터 직접 만들기보다 **Liquibase / Flyway**(스키마 마이그레이션 툴)의 diff·버전관리·update 기능을 활용하고, 그 위에 "변경점 보기 + 클릭 적용" UI만 얹는 것을 권장.

> _(변경 승인 절차 필요 여부, 적용 실패 롤백 처리 등 상세는 이어서 채운다.)_
...
- [ ] 반출(아웃바운드) 차단 범위 확인 — 외부로 나가는 통신(로그 전송·telemetry·`git push` 등)이 어디까지 막히는지 (반입은 가능)
